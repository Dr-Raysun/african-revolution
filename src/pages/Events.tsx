    ...
    <>
            ...
            <div key={num}
              ...
              <div className="lg:col-span-3 p-8">
                <div className="flex flex-wrap items-center gap-3 mb-5">
                  ...
                  <div className="flex items-center gap-1.5 text-[#7C6A86] text-xs">
                    <Icon size={12} /><span>{type}</span>
                  </div>
                </div>
                <h3 className="font-display text-2xl font-bold text-[#2D2438] mb-4 group-hover:text-[#9442C8] transition-colors">{title}</h3>
                <p className="text-[#6B5B75] text-sm leading-relaxed mb-6">{desc}</p>
                <div className="grid sm:grid-cols-2 gap-2 mb-6">
                  {highlights.map((h) => (
                    <div key={h} className="flex items-start gap-2">
                      <span className="shrink-0 mt-0.5" style={{ color: '#9442C8' }}>›</span>
                      <span className="text-[#5B4A66] text-xs">{h}</span>
                    </div>
                  ))}
                </div>
                <Link to="/contact"
      ...
      <section className="py-20 section-divider" style={{ background: '#FFFFFF' }}>
        <div className="max-w-3xl mx-auto px-6 lg:px-8 text-center">
          <h2 className="font-display text-4xl font-bold text-[#2D2438] mb-5">Want to Attend or Partner for an Event?</h2>
